...
public class BookingController {
	...
	public ResponseEntity<Booking> addBooking(@RequestBody Booking booking, @PathVariable LocalDate fromDate,
			@PathVariable LocalDate toDate) throws Exception {
		if (booking == null || toDate == null || fromDate == null || toDate.isBefore(fromDate))
			throw new Exception("Once check the given details");
		else
			return new ResponseEntity<Booking>(bookingService.addBooking(booking, fromDate, toDate),
					HttpStatus.CREATED);
	}
